...
% • Dokumentacja Floorplanu: zrzuty ekranu z narzędzi Vivado/Quartus wizualizujące
% fizyczne rozmieszczenie RO na matrycy FPGA.
% • Schematy RTL: widok zsyntetyzowanej logiki ekstraktora Toeplitza.
% • Raporty Power Analyzer: oszacowany pobór mocy (mW) dla różnych wariantów
% ekstraktorów.
% • Logi i Raporty: kompletne wyniki z testów NIST (SP 800-22 oraz 800-90B) jako
% dowód w aneksie.
% • Dokumentacja Fotograficzna: zdjęcie fizycznego stanowiska (FPGA, pMOD SD,
% setup do testów temperaturowych).

\chapter{Raport}

\textbf{Pytania}
\begin{enumerate}
    \item Kwestia sprzętu: trzymać się swojego czy czekać na Nexys 4?
        \begin{itemize}
            \item Obecnie pracuję na swoim. Zależnie od decyzji dr. Kropidłowskiego pozostanę przy swoim rozwiązaniu lub powtórzę testy na Nexys.
        \end{itemize}

    \item Jak dodać w tekście informację o licencji neoTRNG?

    \item Czy można cytować README repozytorium GitHub?

    \item Czy dodać spis fragmentów kodu tak jak spis rysunków i tabel?

    \item Jak powiedzieć de-biasing / sampling po polsku?
        \begin{itemize}
            \item W tekście użyłem: de-biasing → korekcja źródła / post-processing -> przetwarzanie końcowe / sampling → próbkowanie / CRC → kompresja entropii.
            \item moduł neoTRNG wykorzystuje korekcję źródła z wykorzystaniem ekstraktora von Neumanna
        \end{itemize}

    \item Ile MiB danych do wykonania testów? (zbierałem 10 MiB)
        \begin{itemize}
            \item Będę zbierać po 100 MiB.
        \end{itemize}

    \item NIST SP 800-90B: używać testów IID czy non-IID?
        \begin{itemize}
            \item Wykonywałem testy non-IID, ewentualnie będą do poprawy.
        \end{itemize}

    \item Kto jest autorem neoTRNG?
    
    \item Jak zestawić dane z testów?
        \begin{itemize}
            \item Wyniki testów zestawię jak w Pańskim artykule. Pozostaje kwestia rozkładów C — ustaliliśmy, że jeszcze się Pan nad tym zastanowi.
        \end{itemize}

    \item Publikacje naukowe do uwzględnienia w pracy – sugestie lub wymagania

\end{enumerate}

\textbf{Notatki}
\begin{enumerate}
    \item Najwięcej interwencji z Pańskiej strony będzie wymagał wstęp do pracy. Na razie przygotowałem wstępny zarys, jednak brakuje mu sprecyzowania najważniejszych kwestii oraz nadania tonu reszcie pracy.

    \item W rozdziale teoretycznym wybrałem zagadnienia, które mogłyby być uwzględnione. Ewentualnie proszę o informację, czego brakuje, a co jest zbędne.
\end{enumerate}
...
\textbf{Do zrobienia}
\begin{enumerate}
    \item Dodaj załącznik B z kodem źródłowym
    \item Przerób wykresy .png na .svg
    \item Popraw schematy .svg z Vivado
\end{enumerate}
